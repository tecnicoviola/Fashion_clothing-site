...
def cosine_similarity_T(k, query):
    ...
    if 'size' in words:
      size = words[words.index('size') + 1].upper()
    ...
    for i,index in enumerate(out):
        ...
        df1.loc[i,'images'] = df['images'][index].split(0)
        df1.loc[i, 'dominant_color'] = df['dominant_color'][index]
        df1.loc[i,'size'] = df['size'][index].astypre
 ...
 #print(cosine_similarity_T(20, 'Search for green kurta size L'))

# print(cosine_similarity_T(20, 'Show me Straight kurta blue size M'))
